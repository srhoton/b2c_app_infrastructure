...
resource "aws_ecs_task_definition" "b2c_task_definition" {
  family = "b2c_task_definition-${var.feature}"
  network_mode = "awsvpc"
...
resource "aws_security_group" "b2c_ecs_service" {
  name = "b2c_ecs_service-${var.feature}"
  vpc_id = data.aws_vpc.b2c_vpc.id
  ingress {
    from_port = 22
    to_port = 22
